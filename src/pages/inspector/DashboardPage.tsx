...
import { getAuth, getInspections, searchCompanies, addCompany, createNewInspection, saveInspection, type Company, type Inspection } from '@/lib/store';
...
import { Plus, Search, FileText, ClipboardList, Building2, TrendingUp, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Welcome back, {auth?.name}</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card><CardContent className="flex items-center gap-4 p-4"><TrendingUp className="h-8 w-8 text-info" /><div><p className="text-2xl font-bold">{thisMonth.length}</p><p className="text-sm text-muted-foreground">This Month</p></div></CardContent></Card>
      </div>

      {/* Quick Search / New Inspection */}
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Search className="h-5 w-5" /> Quick Start — Search Company & Inspect</CardTitle></CardHeader>
            ...
            <div className="divide-y rounded-md border">
              {searchResults.map(company => (
                <div key={company.id} className="flex items-center justify-between p-3 hover:bg-muted/50">
                  <div><p className="font-medium">{company.name}</p><p className="text-sm text-muted-foreground">{company.registrationNumber} • {company.address}</p></div>
      ...
      {savedInspections.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-warning" /> Saved Inspections (In Progress)</CardTitle></CardHeader>
          <CardContent>
            <div className="divide-y">
              {savedInspections.map(ins => (
                <div key={ins.id} className="flex items-center justify-between py-3">
                  <div><p className="font-medium">{ins.companyName}</p><p className="text-sm text-muted-foreground">Step {ins.currentStep}/5 • {new Date(ins.updatedAt).toLocaleDateString()}</p></div>
      ...
      {completedInspections.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="divide-y">
              {completedInspections.slice(0, 5).map(ins => (
                <div key={ins.id} className="flex items-center justify-between py-3">
                  <div><p className="font-medium">{ins.companyName}</p><p className="text-sm text-muted-foreground">Signed: {ins.signedAt ? new Date(ins.signedAt).toLocaleDateString() : 'N/A'}</p></div>
                  <div className="flex items-center gap-2">
                    <Badge className={statusColors[ins.status]}>{ins.status}</Badge>
